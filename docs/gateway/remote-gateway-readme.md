...
| ---------------------------------- | ------------------------------------------------------------ |
| `LocalForward 8789 127.0.0.1:8789` | Forwards local port 8789 to remote port 8789                 |
| `ssh -N`                           | SSH without executing remote commands (just port forwarding) |
| `KeepAlive`                        | Automatically restarts tunnel if it crashes                  |
| `RunAtLoad`                        | Starts tunnel when the agent loads                           |

OpenClaw.app connects to `ws://127.0.0.1:8789` on your client machine. The SSH tunnel forwards that connection to port 8789 on the remote machine where the Gateway is running.
