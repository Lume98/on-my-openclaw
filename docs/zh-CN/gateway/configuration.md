...
// ~/.openclaw/openclaw.json
{
  gateway: { port: 8789, auth: { token: "secret" } },
...
- mode：**未设置**（视为"不自动启动"）
- bind：`loopback`
- port：`8789`（WS + HTTP 单端口）

```json5
{
  gateway: {
    mode: "local", // 或 "remote"
    port: 8789, // WS + HTTP 多路复用
    bind: "loopback",
...
- 优先级：`--port` > `OPENCLAW_GATEWAY_PORT` > `gateway.port` > 默认 `8789`。
...
- `gateway.remote.transport` 选择 macOS 远程传输（`ssh` 默认，`direct` 用于 ws/wss）。使用 `direct` 时，`gateway.remote.url` 必须为 `ws://` 或 `wss://`。`ws://host` 默认端口 `8789`。
