...
- `http://<tailscale-ip>:8789/` (or your configured `gateway.controlUi.basePath`)
...
- `http://127.0.0.1:8789/` (on the gateway host)
...
Then point the UI at your Gateway WS URL (e.g. `ws://127.0.0.1:8789`).
...
```text
http://localhost:5173/?gatewayUrl=ws://<gateway-host>:8789
```

Optional one-time auth (if needed):

```text
http://localhost:5173/?gatewayUrl=wss://<gateway-host>:8789#token=<gateway-token>
```
