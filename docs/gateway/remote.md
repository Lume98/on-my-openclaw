...
- The Gateway WebSocket binds to **loopback** on your configured port (defaults to 8789).
...
```bash
ssh -N -L 8789:127.0.0.1:8789 user@host
```

With the tunnel up:

- `openclaw health` and `openclaw status --deep` now reach the remote gateway via `ws://127.0.0.1:8789`.
- `openclaw gateway {status,health,send,agent,call}` can also target the forwarded URL via `--url` when needed.

Note: replace `8789` with your configured `gateway.port` (or `--port`/`OPENCLAW_GATEWAY_PORT`).
...
When the gateway is loopback-only, keep the URL at `ws://127.0.0.1:8789` and open the SSH tunnel first.
...
- Forward `8789` over SSH (see above), then connect clients to `ws://127.0.0.1:8789`.
