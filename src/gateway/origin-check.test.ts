...
describe("checkBrowserOrigin", () => {
  ...
  it("rejects same-origin host matches when legacy host-header fallback is disabled", () => {
    const result = checkBrowserOrigin({
      requestHost: "gateway.example.com:8789",
      origin: "https://gateway.example.com:8789",
    });
  ...
  it('accepts any origin when allowedOrigins includes "*" (regression: #30990)', () => {
    const result = checkBrowserOrigin({
      requestHost: "100.86.79.37:8789",
      origin: "https://100.86.79.37:8789",
      allowedOrigins: ["*"],
    });
    expect(result.ok).toBe(true);
  });
  ...
  it('accepts any origin when allowedOrigins includes "*" alongside specific entries', () => {
    const result = checkBrowserOrigin({
      requestHost: "gateway.tailnet.ts.net:8789",
      origin: "https://gateway.tailnet.ts.net:8789",
      allowedOrigins: ["https://control.example.com", "*"],
    });
    expect(result.ok).toBe(true);
  });
