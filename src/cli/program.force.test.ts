...
describe("gateway --force helpers", () => {
  ...
  it("returns empty list when lsof finds nothing", () => {
    ...
    expect(listPortListeners(8789)).toEqual([]);
  });
  ...
  it("throws when lsof missing", () => {
    ...
    expect(() => listPortListeners(8789)).toThrow(/lsof not found/);
  });
  ...
  it("kills each listener and returns metadata", () => {
    ...
    const killed = forceFreePort(8789);
  ...
  it("retries until the port is free", async () => {
    ...
    const promise = forceFreePortAndWait(8789, {
      timeoutMs: 500,
      intervalMs: 100,
  ...
  it("falls back to fuser when lsof is permission denied", async () => {
    (execFileSync as unknown as Mock).mockImplementation((cmd: string) => {
      ...
      return "8789/tcp: 4242\n";
    });
    tryListenOnPortMock.mockResolvedValue(undefined);

    const result = await forceFreePortAndWait(8789, { timeoutMs: 500, intervalMs: 100 });
  ...
  it("throws when lsof is unavailable and fuser is missing", async () => {
    ...
    await expect(forceFreePortAndWait(8789, { timeoutMs: 200, intervalMs: 100 })).rejects.toThrow(
      /fuser not found/i,
    );
  });
});
...
describe("gateway --force helpers (Windows netstat path)", () => {
  ...
  it("parses PIDs from netstat output correctly", () => {
    (execFileSync as unknown as Mock).mockReturnValue(makeNetstatOutput(8789, 42, 99));
    expect(listPortListeners(8789)).toEqual<PortProcess[]>([{ pid: 42 }, { pid: 99 }]);
  });
  ...
  it("deduplicates PIDs that appear multiple times", () => {
    (execFileSync as unknown as Mock).mockReturnValue(makeNetstatOutput(8789, 42, 42));
    expect(listPortListeners(8789)).toEqual<PortProcess[]>([{ pid: 42 }]);
  });
  ...
  it("throws a descriptive error when netstat fails", () => {
    ...
    expect(() => listPortListeners(8789)).toThrow(/netstat failed/);
  });
