...
const resolveGatewayPortMock = vi.hoisted(() => vi.fn(() => 8789));
...
describe("cli integration: qr + dashboard token SecretRef", () => {
  ...
  it("uses the same resolved token SecretRef for both qr and dashboard commands", async () => {
    ...
    expect(payload.url).toBe("ws://gateway.local:8789");
    ...
    expect(joined).toContain("Dashboard URL: http://127.0.0.1:8789/");
