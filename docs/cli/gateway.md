...
- `--port <port>`: WebSocket port (default comes from config/env; usually `8789`).
...
```bash
openclaw gateway health --url ws://127.0.0.1:8789
```
...
- `gatewayPort` (WebSocket port, usually `8789`)
