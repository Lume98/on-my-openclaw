...
```bash
openclaw node run --host <gateway-host> --port 8789 --display-name "Build Node"
```
...
```bash
# Terminal A (keep running): forward local 18790 -> gateway 127.0.0.1:8789
ssh -N -L 18790:127.0.0.1:8789 user@gateway-host
...
```bash
openclaw node install --host <gateway-host> --port 8789 --display-name "Build Node"
...
```bash
openclaw node run --host <gateway-host> --port 8789
```
