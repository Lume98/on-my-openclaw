...
describe("ports helpers", () => {
  ...
  it("prints an OpenClaw-specific hint when port details look like another OpenClaw instance", async () => {
    ...
    await handlePortError(
      new PortInUseError(8789, "node dist/index.js openclaw gateway"),
      8789,
      "gateway start",
      runtime,
    ).catch(() => {});
  ...
  it("classifies ssh and gateway listeners", () => {
    expect(
      classifyPortListener({ commandLine: "ssh -N -L 8789:127.0.0.1:8789 user@host" }, 8789),
    ).toBe("ssh");
    expect(
      classifyPortListener(
        {
  ...
  it("formats port diagnostics with hints", () => {
    const diagnostics = {
      ...
      listeners: [{ pid: 123, commandLine: "ssh -N -L 8789:127.0.0.1:8789" }],
      hints: buildPortHints([{ pid: 123, commandLine: "ssh -N -L 8789:127.0.0.1:8789" }], 8789),
    };
    const lines = formatPortDiagnostics(diagnostics);
    expect(lines[0]).toContain("Port 8789 is already in use");
    ...
    runCommandWithTimeoutMock.mockImplementation(async (argv: string[]) => {
      ...
      if (command === "ps") {
        if (argv.includes("command=")) {
          return {
            stdout: "node /tmp/openclaw/dist/index.js gateway --port 8789\n",
            stderr: "",
            code: 0,
          };
        }
